...
main(int argc, char **argv) {
    ...
    char fname[] = "stiming_time_sample.csv";
    p_ns =  (uint64_t *)malloc((ntest * narr) * sizeof(uint64_t));

#elif PAPI
    uint64_t *p_ns, ns0, ns1;
    char fname[] = "papi_time_sample.csv";
    int eventset = PAPI_NULL;
    p_ns =  (uint64_t *)malloc((ntest * narr) * sizeof(uint64_t));
    PAPI_library_init(PAPI_VER_CURRENT);
    PAPI_create_eventset(&eventset);
    //PAPI_add_named_event(eventset, "CPU_CLK_THREAD_UNHALTED:THREAD_P");
    PAPI_start(eventset);
#elif CGT
    uint64_t *p_ns, ns0, ns1;
    struct timespec tv1;
    char fname[] = "cgt_time_sample.csv";
    p_ns =  (uint64_t *)malloc((ntest * narr) * sizeof(uint64_t));
#elif WTIME
    uint64_t *p_ns, ns0, ns1;
    struct timespec tv1;
    char fname[] = "wtime_time_sample.csv";
    p_ns =  (uint64_t *)malloc((ntest * narr) * sizeof(uint64_t));
#endif
    ...
    for (int i = 0; i < ntest; i ++) {
        ...
        for (uint64_t k = 0; k < narr; k ++) {
            ...
            register uint64_t a = NSAMP * 2, b, c;
            if (argc < 10) c = 1;
#ifdef STIMING
            _read_ns (ns0);
            _mfence;
    ...
    if (myrank == 0) {
        printf("Finish benchmarking. Writing.\n");
    }
